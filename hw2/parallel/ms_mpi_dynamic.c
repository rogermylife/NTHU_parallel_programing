...
int main(int argc, char** argv) {
    ...
    if (rank==0)
    {
        ...
        do{
            ...
            #if(DEBUG)
            printf("start_row=%d buddyHeightSize=%d flag=%d tag=%d MPI_SOURCE=%d\n", row, info[1], flag, status.MPI_TAG, status.MPI_SOURCE);
            sleep(1);
            #endif
            #if(DEBUG)
    ...
    else
    {
        ...
        while(status.MPI_TAG == DATA_TAG)
        {
            ...
            if(heightSize<=0)
                heightSize=0;
            image = (int*)realloc(image, width*(heightSize)*sizeof(int) );
            for (int j = row, counter = 0; counter < heightSize; ++counter, ++j) {
